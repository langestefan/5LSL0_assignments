...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.Conv2d(in_channels=16, out_channels=16, kernel_size=(5,4), stride=1, padding=(1,0)), # N, 16, 2, 1
            nn.Flatten(),
            # Linear layer to output the mean and the std of the latent space
            nn.Linear(in_features=16, out_features=2),
            x_mean = nn.linear(in_features=64, out_features=2),
            x_std = nn.linear(in_features=64, out_features=2)


        )
        
    def sampling (x_mean,log_x_std):
        # sampling from the latent space
        epsilon = torch.randn_like(x_mean)
        x_std = torch.exp(log_x_std)
